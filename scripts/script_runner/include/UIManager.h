...
public:
    ...
    /**
     * @brief Handle mouse events for scrolling and interaction
     * 
     * Processes mouse wheel events and clicks for scrolling output panes.
     * 
     * @param key The key code returned by getch() (should be KEY_MOUSE)
     * @return true If mouse event was handled
     * @return false If event was not a mouse event or not handled
     */
    bool handleMouseEvent(int key);

    /**
     * @brief Get script to execute from last double-click event
     * 
     * @return std::string Script path to execute, empty if no double-click detected
     */
    std::string getDoubleClickedScript();
    ...
    /**
     * @brief Resize active panel up (expand vertically up)
     * 
     * Expands the active panel in both horizontal directions if possible.
     * 
     * @return true If resize was successful
     * @return false If already at maximum size
     */
    bool resizeActivePaneUp();

    /**
     * @brief Resize active panel down (shrink vertically)
     * 
     * Shrinks the active panel from both horizontal directions if possible.
     * 
     * @return true If resize was successful
     * @return false If already at minimum size
     */
    bool resizeActivePaneDown();

    /**
     * @brief Resize active panel left (expand towards left)
     * 
     * Expands the active panel towards the left direction if possible.
     * 
     * @return true If resize was successful
     * @return false If expansion not possible
     */
    bool resizeActivePaneLeft();

    /**
     * @brief Resize active panel right (expand towards right)
     * 
     * Expands the active panel towards the right direction if possible.
     * 
     * @return true If resize was successful
     * @return false If expansion not possible
     */
    bool resizeActivePaneRight();
...
private:
    ...
    std::chrono::steady_clock::time_point m_lastClickTime;  ///< Time of last mouse click
    int m_lastClickX;          ///< X coordinate of last click
    int m_lastClickY;          ///< Y coordinate of last click
    int m_lastClickedScript;   ///< Index of last clicked script
    std::string m_doubleClickedScript;  ///< Script path from double-click event
    ...
    /**
     * @brief Calculate total wrapped lines for a pane
     * 
     * @param paneIndex Pane index (0 or 1)
     * @return int Total number of wrapped lines
     */
    int calculateWrappedLineCount(int paneIndex) const;
};
